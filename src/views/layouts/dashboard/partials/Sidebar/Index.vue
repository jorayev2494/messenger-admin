...
<template>
  <div class="leftside-menu">
    ...
    <div class="h-100" id="leftside-menu-container" data-simplebar>
      ...
      <ul class="side-nav">
        <span v-for="({ label, items, meta }, idx) of navItems" :key="idx">
          <!-- <pre>1 {{ meta }} {{ $acl.anyPermission(meta.anyPermission ?? []) }} {{ (meta?.anyPermission) ? $acl.anyPermission(meta.anyPermission ?? []) : false }}</pre> -->
          <template v-if="(meta?.anyPermission) ? $acl.anyPermission(meta.anyPermission ?? []) : false">
            <li class="side-nav-title">{{ label }}</li>
